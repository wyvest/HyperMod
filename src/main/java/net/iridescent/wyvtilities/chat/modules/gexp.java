...
public class gexp {
    ...
    {
        try {
            apiObj = JsonParser.parseObj(new BufferedReader(new InputStreamReader(new URL("https://api.hypixel.net/guild?key=" + HyperLeagueConfig.apiKey + "&player=" + Minecraft.getMinecraft().thePlayer.getUniqueID()).openStream())).readLine());
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public int getGEXP() {
        try {
            ...
            gexp = Integer.valueOf((Integer) apiObj.get("response.guild.members." + Minecraft.getMinecraft().thePlayer.getUniqueID() + ".expHistory." + getCurrentESTTime()));
            return gexp;
        } catch(Exception e) {
            e.printStackTrace();
